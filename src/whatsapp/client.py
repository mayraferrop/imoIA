...
class WhatsAppClient:
    ...
    def _archive_whapi(self, group_id: str) -> bool:
        """Arquiva grupo e marca como lido via Whapi.Cloud."""
        success = True

        # Marcar como lido via PATCH /chats/{id}
        try:
            self.mark_chat_as_read(group_id)
        except Exception as e:
            logger.warning(f"Erro ao marcar grupo como lido: {e}")

        # Arquivar via POST /chats/{id} (sem retry para nao sobrecarregar API)
        try:
            self._do_request(
                "POST",
                f"/chats/{group_id}",
            ...
            logger.info(f"Grupo {group_id} arquivado com sucesso (Whapi)")
